...
gulp.task('build-serialport-io', function() {
  gulp.src('serialport-io-bundle-entry.js')
    .pipe(browserify({
      debug: options.param.debug
    }))
    .pipe(rename('serialport-io-bundle.js'))
    .pipe(gulp.dest(options.param.build));
});
...
gulp.task('build', ['build-j5', 'build-firmata', 'build-serialport-io']);
